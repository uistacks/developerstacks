    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\UiStacks\Qas\Controllers\QAsController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Qas::qas.qna') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Qas::qas.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\UiStacks\Qas\Controllers\QAsController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('Qas::qas.name') }}</th>
                                <th>{{ trans('Qas::qas.date') }}</th>
                            ...
                            @if(isset($items))
                                @foreach($items as $item)
                                    @if(isset($item->trans))
                                        <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                            ...
                                            <td>
                                                <a href="{{ action('\UiStacks\Qas\Controllers\QAsController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
                                                &nbsp;<a href="{{ action('\UiStacks\Qas\Controllers\CommentsController@index', $item->id) }}"><i class="fa fa-comment"></i> Comments</a>&nbsp;
                                ...
                                @endforeach
                            @else
                                {{trans('Qas::qas.no_message_found')}}
                            @endif
                            </tbody>
                        </table>
                    </div>
            ...
            @else
                <div class="text-center">
                    <h1>{{ trans('admin.no_results_found') }}
                        {{--<a href="{{ action('\UiStacks\Qas\Controllers\QAsController@index')}}">{{ trans('admin.back') }}</a>--}}
                    </h1>
                </div>
            @endif
        </div>
