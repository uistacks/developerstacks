@php
    ...
    $breadcrumbs[] =  ['url' => url('/').'/'.Lang::getLocale().'/admin/qnas', 'name' => trans('Qas::qas.qna')];
    $action = action('\UiStacks\Qas\Controllers\QAsController@store');
    $method = '';
    $backFieldLabel = trans('admin.add_new_after_save');
    $submitButton = trans('admin.save');
    if(Request::is('*/edit')){
    $pageNameMode = trans('Core::operations.edit');
    $breadcrumbs[] =  ['url' => '', 'name' => trans('Core::operations.edit').' '.trans('Qas::qas.qna')];
    $action = action('\UiStacks\Qas\Controllers\QAsController@update', $item->id);
    $method = 'PATCH';
    $backFieldLabel = trans('admin.back_after_update');
    $submitButton = trans('admin.update');
    }else{
    $breadcrumbs[] =  ['url' => '', 'name' => trans('Core::operations.create').' '.trans('Qas::qas.qna')];
    }
@endphp
