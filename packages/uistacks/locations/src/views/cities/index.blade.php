...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="admin-top-operation">
            <a class="btn btn-default" href="{{ action('\Uistacks\Locations\Controllers\CitiesController@create')}}">Create City</a>
        </div>

        @if($items->count() || $_GET)
            @include('Locations::cities.filter')
        @endif

        @if($items->count())
            <form method="POST" action="{{ action('\Uistacks\Locations\Controllers\CitiesController@bulkOperations')}}" id="bulk" class="form-inline">
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                    <td>
                                       <a href="{{ action('\Uistacks\Locations\Controllers\CitiesController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Uistacks\Locations\Controllers\CitiesController@index')}}">{{ trans('admin.back') }}</a></h1>
                @elseif(Request::is(''.Lang::getlocale().'/admin/countries-areas'))
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Uistacks\Locations\Controllers\CitiesController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Countries::countries.area') }}</a></h1>
                @endif
             </div>
        @endif
    </div>
