@php
    $breadcrumbs = [
    ['url' => '', 'name' => trans('Uiquiz::quiz.quiz')]
    ];
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ action('\UiStacks\Uiquiz\Controllers\TopicsController@create')}}">{{ trans('Core::operations.create') }} {{ trans('Uiquiz::quiz.topic') }}</a>
            </div>
            @if($items->count() || $_GET)
                @include('Quiz::quiz.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ url('/')."/ar/admin/quiz/bulk-operations" }}" id="bulk" class="form-inline">
                                ...
                                @foreach($items as $item)
                                    @if(isset($item->trans))
                                        <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                            ...
                                            <td>
                                                <a href="{{ action('\UiStacks\Uiquiz\Controllers\TopicsController@edit', $item->id) }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
                    ...
                    <div class="form-group">
                        ...
                        <select name="operation" id="operation" class="form-control" required="required">
                            ...
                            <option value="read">{{ trans('Uiquiz::quiz.read') }}</option>
                            <option value="unread">{{ trans('Uiquiz::quiz.unread') }}</option>
