@extends('backend.media.ckeditor.master')
@section('header')
<style type="text/css">
...
<div class="col-xs-6 col-sm-3 placeholder image-item">
  <a id="{{$image->id}}" data-url="/{{$image->thumbnail}}" class="" onclick="returnImageUrl(this)"><img src="/{{$image->thumbnail}}" class="img-responsive"></a>
</div>
@endforeach
...
<script>
    ...
    function returnImageUrl(obj) {
        ...
        fileurl = fileurl.replace('/thumbnail','');

        window.opener.CKEDITOR.tools.callFunction({{ $function }}, fileurl)
