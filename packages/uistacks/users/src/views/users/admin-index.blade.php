    ...
    <div class="card">
        ...
        @else
            <div class="text-center">
                @if($_GET)
                    <h1>{{ trans('admin.no_results_found') }} <a href="{{ action('\Uistacks\Users\Controllers\AdminController@index', $role)}}">{{ trans('admin.back') }}</a></h1>
                @else
                    <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ action('\Uistacks\Users\Controllers\AdminController@create', $role)}}">{{ trans('Users::users.create') }} {{ $itemTitle }}</a></h1>
                @endif
            </div>
        @endif
    </div>
