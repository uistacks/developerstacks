@php
    $breadcrumbs = [
                        ['url' => '', 'name' => trans('Messages::messages.messages')]
    ];
    $dbTable = '';
    if($items->count()){
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="admin-top-operation">
                <a class="btn btn-default" href="{{ url('/').'/'.Lang::getlocale().'/admin/messages/create' }}">{{ trans('Core::operations.create') }} {{ trans('Messages::messages.ad') }}</a>
                {{--{{dd('sd')}}--}}
            </div>
            @if($items->count() || $_GET)
                @include('Messages::messages.filter')
            @endif
            @if($items->count())
                <form method="POST" action="{{ action('\UiStacks\Messages\Controllers\MessagesController@bulkOperations')}}" id="bulk" class="form-inline">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('Messages::messages.name') }}</th>
                            ...
                            @foreach($items as $item)
                                <tr @if($item->trans) data-title="{{ $item->trans->name }}" @endif>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ url('/').'/'.Lang::getlocale().'/admin/messages/'.$item->id.'/edit' }}"><i class="fa fa-edit"></i> {{ trans('Core::operations.edit') }}</a>
            ...
            @else
                <div class="text-center">
                    @if($_GET)
                        <h1>{{ trans('admin.no_results_found') }} <a href="{{ url('/').'/'.Lang::getlocale().'/admin/messages' }}">{{ trans('admin.back') }}</a></h1>
                    @elseif(Request::is(''.Lang::getlocale().'/admin/messages'))
                        <h1>{{ trans('admin.no_data_added_before') }} <a href="{{ url('/').'/'.Lang::getlocale().'/admin/messages/create' }}">{{ trans('Core::operations.create') }} {{ trans('Messages::messages.ad') }}</a></h1>
                    @endif
                </div>
            @endif
        </div>
